...
import com.balancer.service.security.TokenUtils;
...
@RestController
@RequestMapping("${balancer.route.authentication.path}")
public class AuthenticationController {

    @Value("${balancer.token.header}")
    ...
    @RequestMapping(value = "${balancer.route.authentication.refresh}", method = RequestMethod.GET)
    public ResponseEntity<?> authenticationRequest(HttpServletRequest request) {
        String token = request.getHeader(this.tokenHeader);
        String username = this.tokenUtils.getUsernameFromToken(token);
        UserCredentials user = (UserCredentials) this.userDetailsService.loadUserByUsername(username);
